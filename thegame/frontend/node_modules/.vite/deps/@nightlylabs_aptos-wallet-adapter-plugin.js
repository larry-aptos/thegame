...
// node_modules/.pnpm/@nightlylabs+aptos-wallet-adapter-plugin@0.2.12/node_modules/@nightlylabs/aptos-wallet-adapter-plugin/dist/index.mjs
var SHA3 = __toESM(require_sha3(), 1);
var import_bs58 = __toESM(require_bs58(), 1);
var AptosPublicKey = class {
  static default() {
  ...
  static fromBase58(base58string) {
    const bytes = Buffer.from(import_bs58.default.decode(base58string));
...
var NightlyWallet = class {
  ...
  async account() {
    var _a;
    const response = await ((_a = this.provider) == null ? void 0 : _a.aptos.publicKey);
    if (!response)
      throw `${NightlyWalletName} Account Error`;
    return {
  ...
  async disconnect() {
    var _a;
    try {
      await ((_a = this.provider) == null ? void 0 : _a.aptos.disconnect());
    } catch (error) {
      throw error;
    }
  }
  async signTransaction(transaction) {
    var _a;
    try {
      const response = await ((_a = this.provider) == null ? void 0 : _a.aptos.signTransaction(transaction, false));
      if (!response)
        throw `${NightlyWalletName} signAndSubmitTransaction Error`;
      return { hash: Buffer.from(response).toString("hex") };
    } catch (error) {
  ...
  async signAndSubmitTransaction(transaction, options) {
    var _a;
    try {
      const response = await ((_a = this.provider) == null ? void 0 : _a.aptos.signTransaction(transaction, true));
      if (!response)
        throw `${NightlyWalletName} signAndSubmitTransaction Error`;
  ...
  async signMessage(message) {
    ...
    try {
      ...
      const response = await ((_a = this.provider) == null ? void 0 : _a.aptos.signMessage(message.message));
      if (response) {
        return {
  ...
  async network() {
    var _a;
    try {
      const response = await ((_a = this.provider) == null ? void 0 : _a.aptos.network());
      if (!response)
        throw `${NightlyWalletName} Network Error`;
      return {
        name: response.network.toLocaleLowerCase()
      };
    } catch (error) {
      throw error;
    }
...
js-sha3/src/sha3.js:
   ...
   *)
*/
//# sourceMappingURL=@nightlylabs_aptos-wallet-adapter-plugin.js.map
