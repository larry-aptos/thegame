import "./chunk-WNKWOKNR.js";

// node_modules/.pnpm/@tp-lab+aptos-wallet-adapter@1.0.1/node_modules/@tp-lab/aptos-wallet-adapter/dist/index.mjs
var TokenPocketWalletName = "TokenPocket";
var TokenPocketWallet = class {
  constructor() {
    this.name = TokenPocketWalletName;
    this.url = "https://tokenpocket.pro";
    this.providerName = "tokenpocket";
    this.icon = "data:image/png;base64,";
    this.provider = typeof window !== "undefined" ? window.tokenpocket : void 0;
  }
  async connect() {
    var _a, _b;
    try {
      const accountInfo = await ((_b = (_a = this.provider) == null ? void 0 : _a.aptos) == null ? void 0 : _b.connect());
      if (!accountInfo)
        throw `${TokenPocketWalletName} Address Info Error`;
      return accountInfo;
    } catch (error) {
      throw error;
    }
  }
  async account() {
    var _a, _b;
    const response = await ((_b = (_a = this.provider) == null ? void 0 : _a.aptos) == null ? void 0 : _b.account());
    if (!response)
      throw `${TokenPocketWalletName} Account Error`;
    return response;
  }
  async disconnect() {
    var _a, _b;
    try {
      await ((_b = (_a = this.provider) == null ? void 0 : _a.aptos) == null ? void 0 : _b.disconnect());
    } catch (error) {
      throw error;
    }
  }
  async signTransaction(transaction, options) {
    var _a, _b;
    try {
      const response = await ((_b = (_a = this.provider) == null ? void 0 : _a.aptos) == null ? void 0 : _b.signTransaction(transaction, options));
      if (!response) {
        throw new Error("signTransaction Error");
      }
      return response;
    } catch (error) {
      throw error;
    }
  }
  async signAndSubmitTransaction(transaction, options) {
    var _a, _b;
    try {
      const response = await ((_b = (_a = this.provider) == null ? void 0 : _a.aptos) == null ? void 0 : _b.signAndSubmitTransaction(
        transaction,
        options
      ));
      if (response) {
        return response;
      } else {
        throw `${TokenPocketWalletName} Sign and submit Transaction failed`;
      }
    } catch (error) {
      const errMsg = error.message;
      throw errMsg;
    }
  }
  async signMessage(message) {
    var _a, _b;
    try {
      if (typeof message !== "object" || !message.nonce) {
        `${TokenPocketWalletName} Invalid signMessage Payload`;
      }
      const response = await ((_b = (_a = this.provider) == null ? void 0 : _a.aptos) == null ? void 0 : _b.signMessage(message));
      if (response) {
        return response;
      } else {
        throw `${TokenPocketWalletName} Sign Message failed`;
      }
    } catch (error) {
      const errMsg = error.message;
      throw errMsg;
    }
  }
  async network() {
    var _a, _b;
    try {
      const response = await ((_b = (_a = this.provider) == null ? void 0 : _a.aptos) == null ? void 0 : _b.network());
      if (!response)
        throw `${TokenPocketWalletName} Network Error`;
      return {
        name: response
      };
    } catch (error) {
      throw error;
    }
  }
  async onNetworkChange(callback) {
  }
  async onAccountChange(callback) {
  }
};
export {
  TokenPocketWallet,
  TokenPocketWalletName
};
//# sourceMappingURL=@tp-lab_aptos-wallet-adapter.js.map
